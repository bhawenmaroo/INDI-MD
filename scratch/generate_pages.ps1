$indexContent = Get-Content -Raw -Path "index.html"

# Extract parts using RegEx
$headRegex = '(?s)(<head>.*?</head>)'
$headerRegex = '(?s)(<header.*?</header>)'
$footerRegex = '(?s)(<footer.*?</footer>)'
$whatsappRegex = '(?s)(<a href="https://wa\.me/918902052927".*?</a>)'
$scriptsRegex = '(?s)(<!-- Scripts -->.*?</body>)'

$headTemplate = [regex]::Match($indexContent, $headRegex).Groups[1].Value
$headerTemplate = [regex]::Match($indexContent, $headerRegex).Groups[1].Value
$footerTemplate = [regex]::Match($indexContent, $footerRegex).Groups[1].Value
$whatsappTemplate = [regex]::Match($indexContent, $whatsappRegex).Groups[1].Value
$scriptsTemplate = [regex]::Match($indexContent, $scriptsRegex).Groups[1].Value
...
function Generate-Page {
    ...
    $head = $headTemplate -replace '(?s)<title>.*?</title>', "<title>$Title</title>"
    $head = $head -replace '(?s)<meta name="description" content=".*?">', "<meta name=`"description`" content=`"$MetaDesc`">"
...
Generate-Page "about-us.html" "About IndiBiotek | India's Pioneer Biotech Startup" "IndiBiotek Private Limited — founded to bring indigenous biotech manufacturing to India." $about_us
Generate-Page "research-and-development.html" "Research & Development | IndiBiotek Biotech Innovation" "IndiBiotek's R&D spans AI diagnostics, CRISPR, microfluidics, and agri-biotech." $research_and_development
Generate-Page "lifesciences.html" "Lifesciences Division | Diagnostics, Kits & Reagents | IndiBiotek" "AI-based diagnostics, molecular kits, ELISA, CLIA, rapid tests, POC devices." $lifesciences
Generate-Page "agri.html" "Agri Division | Biofertilizers, Biopesticides & Bioinputs | IndiBiotek" "IndiBiotek's Agri division offers biofertilizers, bioinsecticides, biopesticides." $agri
Generate-Page "scientific.html" "Scientific Division | Reagents, Chemicals & Lab Consumables | IndiBiotek" "IndiBiotek's Scientific division supplies indigenous reagents, buffers, chemicals." $scientific
Generate-Page "services.html" "Services | IndiBiotek Private Limited" "IndiBiotek offers research collaboration, diagnostic development, reagent customisation." $services
Generate-Page "contact-us.html" "Contact IndiBiotek | Get In Touch" "Contact IndiBiotek Private Limited — Kolkata headquarters." $contact_us
Generate-Page "privacy-policy.html" "Privacy Policy | IndiBiotek Private Limited" "Privacy Policy of IndiBiotek Private Limited." $privacy_policy
