...
# Read index.html to extract head, header, footer and whatsapp float
...
whatsapp_match = re.search(r'(<a href="https://wa\.me/918902052927".*?</a>)', index_content, re.DOTALL)
whatsapp_template = whatsapp_match.group(1) if whatsapp_match else ""

# Extract scripts
scripts_match = re.search(r'(<!-- Scripts -->.*?</body>)', index_content, re.DOTALL)
scripts_template = scripts_match.group(1) if scripts_match else ""

def generate_page(filename, title, meta_desc, content_html, body_class=""):
    # update title and meta description in head
    head = re.sub(r'<title>.*?</title>', f'<title>{title}</title>', head_template)
...
pages = [
    ('about-us.html', 'About IndiBiotek | India\'s Pioneer Biotech Startup', 'IndiBiotek Private Limited — founded to bring indigenous biotech manufacturing to India.', about_us),
    ('research-and-development.html', 'Research & Development | IndiBiotek Biotech Innovation', 'IndiBiotek\'s R&D spans AI diagnostics, CRISPR, microfluidics, and agri-biotech.', research_and_development),
    ('lifesciences.html', 'Lifesciences Division | Diagnostics, Kits & Reagents | IndiBiotek', 'AI-based diagnostics, molecular kits, ELISA, CLIA, rapid tests, POC devices.', lifesciences),
    ('agri.html', 'Agri Division | Biofertilizers, Biopesticides & Bioinputs | IndiBiotek', 'IndiBiotek\'s Agri division offers biofertilizers, bioinsecticides, biopesticides.', agri),
    ('scientific.html', 'Scientific Division | Reagents, Chemicals & Lab Consumables | IndiBiotek', 'IndiBiotek\'s Scientific division supplies indigenous reagents, buffers, chemicals.', scientific),
    ('services.html', 'Services | IndiBiotek Private Limited', 'IndiBiotek offers research collaboration, diagnostic development, reagent customisation.', services),
    ('contact-us.html', 'Contact IndiBiotek | Get In Touch', 'Contact IndiBiotek Private Limited — Kolkata headquarters.', contact_us),
    ('privacy-policy.html', 'Privacy Policy | IndiBiotek Private Limited', 'Privacy Policy of IndiBiotek Private Limited.', privacy_policy),
]

for filename, title, meta_desc, content_html in pages:
    generate_page(filename, title, meta_desc, content_html)
    print(f"Generated {filename}")

# Also let's update CSS to add form focus styles and background animations for divisions
css_append = """
...
/* Need to ensure nav text is readable on white pages */
